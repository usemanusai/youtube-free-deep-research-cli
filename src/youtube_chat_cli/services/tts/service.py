...
try:
    import torch
    # Try importing chatterbox with error handling for dependency issues
    try:
        ...
        logger.info(f"Chatterbox TTS not available due to dependency issues: {e}")
        CHATTERBOX_AVAILABLE = False
except ImportError:
    logger.info("Chatterbox TTS not available - PyTorch not installed")
