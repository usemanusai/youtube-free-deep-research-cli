...
# Define custom exceptions to avoid relative imports
...
class LLMService:
    ...
    @staticmethod
    def _resolve_role_model(role: Optional[str], default_model: str) -> str:
        """Resolve a model for a specific agent role, falling back to defaults.

        Priority:
        1) AGENTS_CONFIG_JSON (JSON string) with keys like planner/orchestrator/cortex/executor and utils.hopper/utils.outputter
        2) Role-specific env vars (e.g., AGENT_PLANNER_MODEL, AGENT_EXECUTOR_MODEL, AGENT_UTILS_HOPPER_MODEL)
        3) default_model
        """
        if not role:
        ...
        # 1) JSON config (AGENTS_CONFIG_JSON)
        cfg_json = os.getenv('AGENTS_CONFIG_JSON')
        if cfg_json:
            try:
                cfg = json.loads(cfg_json)
                # direct roles
                if role_key in cfg and isinstance(cfg[role_key], dict):
                    m = cfg[role_key].get('model')
                    if m:
                        return m
                # utils sub-roles
                if role_key in ('hopper', 'outputter') and isinstance(cfg.get('utils'), dict):
                    util = cfg['utils'].get(role_key)
                    if isinstance(util, dict):
                        m = util.get('model')
                        if m:
                            return m
            except Exception:
                # ignore parse errors and continue to env fallbacks
                pass

        # 2) Role-specific environment variables
        env_map = {
            'planner': 'AGENT_PLANNER_MODEL',
            'orchestrator': 'AGENT_ORCHESTRATOR_MODEL',
            'cortex': 'AGENT_CORTEX_MODEL',
            'executor': 'AGENT_EXECUTOR_MODEL',
            'hopper': 'AGENT_UTILS_HOPPER_MODEL',
            'outputter': 'AGENT_UTILS_OUTPUTTER_MODEL',
        }
        env_name = env_map.get(role_key)
        if env_name:
    ...
    def __init__(self, api_key: str = None, model: str = None, role: Optional[str] = None):
        """Initialize the LLM service.

        Args:
            api_key: OpenRouter API key. If None, attempts to load from environment.
            model: Model name for OpenRouter API. If None, uses per-role env (if provided) or LLM_MODEL, then defaults.
            role: Optional agent role selector (e.g., 'planner', 'executor', 'hopper').
        """
        if api_key is None:
        ...
        default_model = model or (env_llm_model if env_llm_model else 'z-ai/glm-4.5-air:free')
        logger.info(f"env LLM_MODEL='{env_llm_model}' -> default_model='{default_model}'")
        selected_model = self._resolve_role_model(role, default_model)

        self.api_key = api_key
        self.model = selected_model
        self.role = role

        logger.info(f"LLMService initialized with model='{selected_model}' role='{role or ''}'")
