...
try:
    try:
        ...
        # Fallback for when cli.py is executed as a script from inside the package dir
        # so absolute import 'youtube_chat_cli_main.*' is not resolvable
        from tts_bridge_client import TTSBridgeClient  # type: ignore
...
class TTSService:
    ...
    def __init__(self, language: str = 'en'):
        ...
        # Detect bridge availability via marker file if import check failed
        if not self.tts_bridge_available:
            try:
                from pathlib import Path
                if Path('.python311_path').exists():
                    self.tts_bridge_available = True
                    logger.info("✅ TTS Bridge detected via .python311_path marker")
            except Exception:
                pass

        # Ensure attribute exists even if import-time check failed
        if not hasattr(self, 'tts_bridge'):
    ...
    def generate_audio(self, text: str, output_file: str = "overview.wav",
        ...
        # FORCE: Chatterbox when requested (default ON for podcast path)
        if os.getenv("TTS_FORCE_CHATTERBOX", "1") == "1":
            try:
                logger.info("🎯 Forcing Chatterbox via Python 3.11 bridge...")
                return self.generate_audio_chatterbox_bridge(text, output_file, exaggeration=0.5)
            except Exception as e:
                logger.warning(f"Forced Chatterbox failed: {e}")
                raise
