...
class GoogleDriveService:
        ...
        if not self.creds or not self.creds.valid:
            ...
            if not self.creds:
                ...
                try:
                    ...
                    # Use fixed port 8080 (must be configured in Google Cloud Console)
    ...
    def check_for_changes(self, folder_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Check for new or modified files in Google Drive.

        This method compares current files with the database to detect changes.

        Args:
            folder_id: Folder ID to monitor (None = use config default)

        Returns:
            List of changed files (new or modified)
        """
        ...
        try:
            ...
            current_files = self.list_files(folder_id=folder_id)
            ...
            for file in current_files:
                ...
                db_file = self.db.get_gdrive_file(file_id)

                if not db_file:
                    # New file
                    logger.info(f"New file detected: {file['name']} ({file_id})")
                    changed_files.append(file)

                    # Add to database
                    self.db.upsert_gdrive_file(
                        file_id=file_id,
                        name=file['name'],
                ...
                elif db_file['modified_time'] != modified_time or \
                     db_file['md5_checksum'] != md5_checksum:
                    # Modified file
                    logger.info(f"Modified file detected: {file['name']} ({file_id})")
                    changed_files.append(file)
            ...
            raise GoogleDriveError(f"Failed to check for changes: {e}")

    def add_files_to_queue(self, files: List[Dict[str, Any]], priority: int = 0) -> int:
        """
        ...
        for file in files:
            try:
                ...
                logger.info(f"Added to queue: {file['name']} (Queue ID: {queue_id})")
                count += 1

            except Exception as e:
                logger.error(f"Failed to add file to queue: {file['name']}: {e}")
...
class GoogleDriveWatcher:
    """
    Background service for monitoring Google Drive folder changes.

    This class periodically checks for new or modified files and adds them
    ...
    def watch(self) -> int:
        """
        Check for changes and add new files to queue.
        ...
        try:
            # Check for changes
            changed_files = self.drive_service.check_for_changes()
            ...
            count = self.drive_service.add_files_to_queue(changed_files)

            self.last_check = datetime.now()
            logger.info(f"✅ Added {count} files to processing queue")

            return count

        except Exception as e:
            logger.error(f"Google Drive watch failed: {e}")
            return 0


# Global service instances
_gdrive_service: Optional[GoogleDriveService] = None
_gdrive_watcher: Optional[GoogleDriveWatcher] = None
...
def get_gdrive_service() -> GoogleDriveService:
    ...
    if _gdrive_service is None:
        ...
        logger.info("Google Drive service instance created")

    return _gdrive_service


def get_gdrive_watcher() -> GoogleDriveWatcher:
    """
    Get the global Google Drive watcher instance.

    Returns:
        GoogleDriveWatcher instance
    """
    global _gdrive_watcher

    if _gdrive_watcher is None:
        _gdrive_watcher = GoogleDriveWatcher()
        logger.info("Google Drive watcher instance created")

    return _gdrive_watcher
