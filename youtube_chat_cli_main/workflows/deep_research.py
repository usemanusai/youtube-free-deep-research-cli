            ...
            def _gen_enh(prompt: str, sys: str, temp: float):
                ...
                try:
                    ...
                    if br:
                        br.on_failure()
                    raise
            prompt_str = f"Original topic: {state['topic']}\nReturn improved topic only."
            try:
                enhanced_out = _gen_enh(
                    prompt=prompt_str,
                    sys=ENHANCER_SYSTEM,
                    temp=0.9,
                )
                enhanced_tmp = (enhanced_out or "").strip()
                # If circuit is open, _gen_enh echoes prompt; normalize to original topic
                enhanced = state['topic'] if enhanced_tmp == prompt_str else (enhanced_tmp or state['topic'])
            except Exception:
                enhanced = state['topic']
