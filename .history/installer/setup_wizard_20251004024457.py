...
def main() -> int:
    ...
    # Interactive/Non-interactive detection and confirmation
    interactive = sys.stdin.isatty() and sys.stdout.isatty() and not os.environ.get("INSTALLER_NONINTERACTIVE")
    if interactive:
        if not questionary.confirm("Proceed with installation?").ask():
            console.print("[yellow]Cancelled by user")
            return 0
    else:
        console.print("[cyan]Non-interactive mode detected. Proceeding with defaults.")
