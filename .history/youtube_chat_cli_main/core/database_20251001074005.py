...
class Database:
    ...
    def get_queue_statistics(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            # Get failed items with high retry count
            cursor.execute("""
                SELECT COUNT(*) as count
                FROM processing_queue
                WHERE status = 'failed' AND retry_count >= 3
            """)
            failed_high_retry = cursor.fetchone()['count']

            return {
                'total': total,
                'by_status': status_counts,
                'failed_high_retry': failed_high_retry
            }
    ...
    def add_vector_metadata(
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("""
                ...
                (vector_id, file_id, chunk_index, metadata)
                VALUES (?, ?, ?, ?)
            """, (
                vector_id,
                file_id,
