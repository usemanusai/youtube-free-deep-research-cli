...
class Config:
    ...
    def openrouter_api_key(self) -> Optional[str]:
        """OpenRouter API key for LLM access (single key for backward compatibility)."""
        return os.getenv('OPENROUTER_API_KEY')

    @property
    def openrouter_api_keys(self) -> list[str]:
        """
        OpenRouter API keys for rotation (supports multiple free-tier accounts).

        Supports two formats:
        1. Comma-separated list: OPENROUTER_API_KEYS=key1,key2,key3
        2. Numbered keys: OPENROUTER_API_KEY, OPENROUTER_API_KEY_2, OPENROUTER_API_KEY_3, etc.

        Returns:
            List of API keys, with duplicates removed
        """
        keys = []
        ...
        # Option 2: Numbered keys (OPENROUTER_API_KEY, OPENROUTER_API_KEY_2, etc.)
        ...
        for i in range(2, 101):
            ...
            if key and key not in keys:
                keys.append(key)
            elif not key and i > 10:  # Stop checking after 10 consecutive missing keys
                break
    ...
    def has_llm_api_key(self) -> bool:
        ...
        return bool(self.openrouter_api_keys or self.openai_api_key)
