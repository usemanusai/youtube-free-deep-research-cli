...
export function SystemStatus() {
  ...
  const refreshStatus = async () => {
    ...
    try {
      // Refresh system status from real API
      await refetch()
      await refetchVerify()

      // Update metrics (mock for now - TODO: add metrics endpoint)
      setMetrics(prev => ({
        ...prev,
  ...
  // Convert API data to ServiceStatus format
  const services: ServiceStatus[] = systemStatus ? Object.entries(systemStatus.services).map(([name, info]: [string, any]) => ({
    name: name.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
    status: info.status as ServiceStatus['status'],
    lastCheck: new Date(),
    error: info.error,
    type: info.type,
