...
class TTSService:
    ...
    def generate_audio_chatterbox(self, text: str, output_file: str = "chatterbox_output.wav",
        ...
        try:
            ...
            # Determine if we should use multilingual model (future enhancement)
            config = get_tts_config_manager().get_library_config("chatterbox")
            use_multilingual = config.get("use_multilingual", False)

            if use_multilingual:
                # Use multilingual model for advanced features
                from chatterbox.mtl_tts import ChatterboxMultilingualTTS
                if not isinstance(self.chatterbox_model, ChatterboxMultilingualTTS):
                    self.chatterbox_model = ChatterboxMultilingualTTS.from_pretrained(device=device)
                    logger.info("Switched to Chatterbox multilingual model")

                # Generate with multilingual features
                wav_data = self.chatterbox_model.generate(text, language_id="en")
            else:
                # Standard generation with emotion control
                wav_data = self.chatterbox_model.generate(text, emotion=emotion_level)

            # Save audio file
            if os.path.exists(output_file):
                os.remove(output_file)

            # Chatterbox typically returns numpy array or torch tensor
            if hasattr(wav_data, 'numpy'):
                audio_array = wav_data.numpy()
            elif hasattr(wav_data, 'cpu'):
                audio_array = wav_data.cpu().numpy()
            else:
            ...
            try:
                ...
                sf.write(output_file, audio_array, 22050)  # Chatterbox default sample rate
            except ImportError:
                # Fallback: use scipy to write WAV
                from scipy.io.wavfile import write
                write(output_file, 22050, audio_array)
