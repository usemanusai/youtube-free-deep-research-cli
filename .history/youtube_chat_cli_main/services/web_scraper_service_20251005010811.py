...
class WebScraperService:
            ...
            with sync_playwright() as p:
                for i in range(attempts):
                    try:
                        ...
                        # Site-specific waits (selectors)
                        try:
                            self._apply_site_waits(page, url, cfg, timeout_s)
                        except Exception:
                            pass
    ...
    def _normalized_request_url(self, u: str) -> str:
        """Normalize root URLs so mocks like responses can match (e.g., add trailing '/')."""
        try:
            parts = urlsplit(u)
            if (parts.path or "") == "" and (parts.query or "") == "" and (parts.fragment or "") == "":
    ...
    def scrape(self, url: str, depth: int = 1, max_pages: int = 50, timeout_s: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            if not html_main:
                req_url = self._normalized_request_url(url)
                resp = request_with_retry("GET", req_url, headers={"User-Agent": ua}, timeout=timeout_s)
                status = resp.status_code
                text = _html_to_text(resp.text or "") if resp.status_code and status < 400 else ""
            else:
                status = 200
            ...
            if depth and depth > 1 and (html_main or status == 200):
                ...
                for link in links[: max_pages - 1]:
                    try:
                        ...
                        if not html2:
                            link_req = self._normalized_request_url(link)
                            r2 = request_with_retry("GET", link_req, headers={"User-Agent": ua}, timeout=timeout_s)
                            text2 = _html_to_text(r2.text or "") if (r2.status_code and r2.status_code < 400) else ""
