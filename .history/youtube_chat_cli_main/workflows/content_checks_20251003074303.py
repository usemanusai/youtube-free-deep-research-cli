...
try:
    ...
    def _build_cc_graph2():
        ...
        def n_execute(state: CCState) -> CCState:
            ...
            return {**state, 'conv': conv, 'insights': insights, 'started_at': started, 'citations': _citations_from_conv(conv)}
        ...
        def n_duplicate_check(state: CCState) -> CCState:
            ...
            return {**state, 'decision': decision, 'evidence': ev}
        ...
        def n_synthesize(state: CCState) -> CCState:
            ...
            return {**state, 'output': {'synthesis': synthesis}}
        ...
        def n_refine(state: CCState) -> CCState:
            ...
            return {**state, 'output': {'refined_prompt': refined}, 'insights': insights, 'loop_count': loops, 'conv': rerun}
...
def run(topic: str, max_loops: Optional[int] = None) -> Dict[str, Any]:
    ...
    # Persist content-checks session
    try:
        import uuid
        ...
        # store compact pieces from conversation
        tx = conv.get("transcript", []) if isinstance(conv, dict) else []
        for t in tx[:10]:
