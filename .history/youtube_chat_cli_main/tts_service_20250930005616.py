...
class TTSService:
    ...
    def _generate_podcast_segments(self, podcast_script: str) -> list:
        ...
        # Split script by speaker indicators
        # Look for patterns like **Host (H):** or **H:** etc.
        pattern = r'\*\*([^*]+?):\*\*\s*(.+?)(?=\*\*[^*]+?:\*\*|$)'
        matches = re.findall(pattern, podcast_script, re.MULTILINE | re.DOTALL)

        if not matches:
            # Try simpler patterns like **Host:** or **Expert:**
        ...
        # Define voice mapping for speakers - using available gTTS voices
        voice_map = {
            'host': 'en',         # Standard English
            'expert': 'com',       # Commonwealth English (different accent)
            'h': 'en',            # Short forms
            'e': 'com',
            'alex': 'com',
            'chen': 'com'
        ...
        try:
            for speaker_info, text in matches:
                ...
                try:
                    self.generate_audio(text.strip(), temp_file.name, voice=voice)
                    segments.append((speaker_info, voice, text.strip(), temp_file.name))
                except Exception as e:
