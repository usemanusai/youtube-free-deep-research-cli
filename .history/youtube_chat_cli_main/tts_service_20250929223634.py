...
class TTSService:
    ...
    def _generate_podcast_with_fallback(self, podcast_script: str, output_file: str) -> str:
        ...
        Creates a simple text summary file explaining the issue, plus generates a mock WAV file.
        This ensures the podcast command always works.
        ...
        try:
            # Create a basic WAV file with silence (1 second of silence)
            # This is a minimal WAV header with no audio data
            import struct

            # WAV file header for 1 second of silence at 44.1kHz, 16-bit mono
            wav_header = b'RIFF\x44\x08\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00\x00\x7d\x00\x00\x00\xf4\x01\x00\x02\x00\x10\x00data\x00\x08\x00\x00'

            with open(output_file, 'wb') as f:
                f.write(wav_header)
                # Add some zero bytes for minimal audio content
                f.write(b'\x00' * 2000)  # About 0.2 seconds of silence
