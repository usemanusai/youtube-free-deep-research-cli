...
class TTSService:
    ...
    def _generate_podcast_segments(self, podcast_script: str) -> list:
        ...
        # Split script by speaker indicators - more flexible patterns
        # Look for **Speaker:** or **Speaker (Name):** formats
        pattern = r'\*\*([^*]+?):\*\*\s*([^[*]*(?:\*[^*])*\*\*)*(.+?)(?=\*\*[^*]+?:\*\*|Generated on|$|$)'
        matches = re.findall(pattern, podcast_script, re.MULTILINE | re.DOTALL)

        if not matches:
            # Try simpler pattern without end bound
            pattern = r'\*\*([^*]+?):\*\*\s*(.+?)(?=\*\*[^*]+?:\*\*|Generated on|$)'
            matches = re.finditer(pattern, podcast_script, re.MULTILINE | re.DOTALL)
            matches = [(m.group(1), m.group(2)) for m in matches]
        ...
        try:
            for speaker_info, text in matches:
                ...
                try:
                    self.generate_audio(text.strip(), temp_file.name, voice=voice_params[0], slow=voice_params[1])
                    segments.append((speaker_info, voice_params, text.strip(), temp_file.name))
                except Exception as e:
    ...
    def _combine_audio_segments(self, segments: list, output_file: str) -> str:
        ...
        try:
            ...
            raise APIError("pydub required for multi-speaker podcast generation")
        ...
        # Add small pause between speakers (500ms silence)
        # This isn't directly supported, but we can insert silence using AudioSegment.silent
