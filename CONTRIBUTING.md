...
- Preferred: uv
  - Lock: `uv lock`
  - Run: `uv run --with dev pytest -q`
- Fallback: pip
  - `python -m pip install -r requirements.txt -r youtube_chat_cli_main/api_requirements.txt`

## HTTP & Testing Notes

- Centralized HTTP client lives in `youtube_chat_cli_main/core/http_client.py`
- For httpx mocking, prefer `respx` with the provided `respx_mock` fixture
- If a test needs a fresh `httpx.Client` under respx, call `reset_httpx_client()` first
